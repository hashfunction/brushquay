...
function Add-GuiDisplayTypes {
    if('BristluneGui.DisplayModes' -as [type]){return}
    Add-Type -TypeDefinition @'
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Runtime.InteropServices;
namespace BristluneGui {
...
function Get-GuiDisplayState([string]$Device){
    Add-GuiDisplayTypes
    if(-not $Device){$Device=[BristluneGui.DisplayModes]::Primary()}
    return @{device=$Device;current=[BristluneGui.DisplayModes]::Current($Device);modes=[BristluneGui.DisplayModes]::Supported($Device)}
}
function Set-GuiDisplayMode([string]$Device,$Mode,[int]$Flags){
    return [BristluneGui.DisplayModes]::Change($Device,$Mode,$Flags)
}
